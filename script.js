...
document.addEventListener('DOMContentLoaded', () => {
        ...
        toolsToRender.forEach((tool, index) => {
            ...
            card.innerHTML = `
                <div class="card-header">
                    <h3>${tool.name}</h3>
                    <button class="favorite-btn ${isFavorited ? 'favorited' : ''}" data-tool-name="${tool.name}">
    ...
    const filterAndRender = () => {
        ...
        if (isFavoritesView) {
            filteredTools = filteredTools.filter(tool => favorites.includes(tool.name));
        }
        ...
        filteredTools.sort((a, b) => favorites.includes(b.name) - favorites.includes(a.name));
    ...
    searchBar.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            ...
            if (isFavoritesView) {
                currentFilteredTools = currentFilteredTools.filter(tool => favorites.includes(tool.name));
            }
